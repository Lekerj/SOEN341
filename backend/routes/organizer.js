...
const validateEventData = (data) => {
  ...
  if (isNaN(priceFloat) || priceFloat < 0) {
    ...
    errors.push(
      "price: Must have at most two decimal places (e.g., 10 or 10.99)."
    );
  }
  ...
  if (!category || !validCategories.includes(category)) {
    errors.push(
      `category: Required, must be one of: ${validCategories.join(", ")}.`
    );
  }
...
router.post("/events", requireApprovedOrganizer, (req, res) => {
  ...
  // 2a. Get organizer's organization name
  db.query(
    "SELECT organization_id FROM organizer_requests WHERE user_id = ? AND status = 'approved'",
    [organizer_Id],
    (err, requestRows) => {
      if (err) {
        ...
        return res.status(500).json({
          ...
          message: "Failed to fetch organizer information.",
        });
      }
      ...
      if (!requestRows || requestRows.length === 0) {
        return res.status(400).json({
          success: false,
          error: "Invalid Data",
          message: "No approved organization found for this organizer.",
        });
      }

      const orgId = requestRows[0].organization_id;
        ...
        (orgErr, orgRows) => {
          if (orgErr) {
            ...
            return res.status(500).json({
              success: false,
              error: "Internal Server Error",
              message: "Failed to fetch organization name.",
            });
          }
          ...
          db.query(sql, params, (err, result) => {
            if (err) {
              ...
              return res.status(500).json({
                success: false,
                error: "Internal Server Error",
                message: "Failed to create event due to a database issue.",
              });
            }
...
router.put("/events/:id", requireApprovedOrganizer, (req, res) => {
  ...
  if (eventData.title !== undefined) {
    if (!eventData.title) errors.push("title: Required if provided.");
      ...
      errors.push("title: Must be string <=255 chars.");
  }
  if (
    eventData.description !== undefined &&
    eventData.description &&
    eventData.description.length > 5000
  ) {
  ...
  if (eventData.location !== undefined) {
    if (!eventData.location) errors.push("location: Required if provided.");
      ...
      eventData.location.length > 255
    )
      errors.push("location: Must be string <=255 chars.");
  }
  if (eventData.organization !== undefined) {
    if (!eventData.organization)
      errors.push("organization: Required if provided.");
    else if (
      typeof eventData.organization !== "string" ||
      eventData.organization.length > 100
    )
      errors.push("organization: Must be string <=100 chars.");
  }
  if (eventData.event_date !== undefined) {
    if (
      !/^\d{4}-\d{2}-\d{2}$/.test(eventData.event_date) ||
      isNaN(Date.parse(eventData.event_date))
    ) {
  ...
  if (eventData.event_time !== undefined) {
    if (!/^\d{2}:\d{2}:\d{2}$/.test(eventData.event_time))
      errors.push("event_time: Must be HH:MM:SS");
  }
  if (eventData.capacity !== undefined) {
    const cInt = parseInt(eventData.capacity);
    if (isNaN(cInt) || cInt < 1 || cInt > 10000)
      errors.push("capacity: 1-10000");
  }
  if (eventData.price !== undefined) {
    const pStr = String(eventData.price);
    const pF = parseFloat(pStr);
    if (isNaN(pF) || pF < 0) errors.push("price: Non-negative number");
    else if (!/^\d+(\.\d{1,2})?$/.test(pStr))
      errors.push("price: Max two decimals");
  }
  ...
  db.query(
    ...
    (err, rows) => {
      if (err) {
        console.error("DB ownership check error:", err);
        return res
          .status(500)
      ...
      if (rows[0].organizer_id !== organizerId)
      ...
      [
        ...
        if (eventData[field] !== undefined) {
          ...
          values.push(eventData[field] || null);
        }
      });
      ...
      values.push(eventId);
      const updateSql = `UPDATE events SET ${setClauses.join(
        ", "
      )} WHERE id = ?`;
      db.query(updateSql, values, (e2) => {
        if (e2) {
          console.error("DB event update error:", e2);
          return res
            .status(500)
        ...
        db.query("SELECT * FROM events WHERE id = ?", [eventId], (e3, rs2) => {
          if (e3) {
            console.error("DB fetch updated event error:", e3);
            return res
              .status(500)
              .json({ success: false, error: "Fetch failed" });
          }
          if (!rs2.length)
            return res
              .status(500)
              .json({ success: false, error: "Updated event not retrievable" });
          return res
            .status(200)
            .json({ success: true, message: "Event updated", event: rs2[0] });
        });
      });
    }
  );
...
router.post("/request", requireAuth, (req, res) => {
  ...
  const { organization_id, new_organization_name, category, details } =
    req.body || {};

  console.log(`[ORGANIZER REQUEST] POST /api/organizer/request called`);
  console.log(`[ORGANIZER REQUEST] User ID: ${userId}`);
  console.log(`[ORGANIZER REQUEST] Body:`, req.body);

  if (!organization_id && !new_organization_name) {
    return res.status(400).json({
      success: false,
      error: "Provide organization_id or new_organization_name",
    });
  }

  // Check for existing pending request - user should use PATCH to modify it instead
  db.query(
    "SELECT organizer_auth_status FROM users WHERE id = ?",
    [userId],
    (err, userRows) => {
      if (err) {
      ...
      console.log(`[ORGANIZER] User ${userId} attempting to create organizer request. Current status: ${userRows[0]?.organizer_auth_status}`);
      console.log(`[ORGANIZER] Request data: organization_id=${organization_id}, new_organization_name=${new_organization_name}, category=${category}`);
      ...
      if (currentStatus === "pending") {
        return res.status(400).json({
          success: false,
          error:
            "You already have a pending request. Use PATCH /api/organizer/request/:id to modify it.",
        });
      }
      ...
      function finalizeWithOrgId(orgId, orgName) {
        ...
        db.query(
          insertRequest,
          [userId, orgId, reqType, details || null],
          (err) => {
            if (err) {
            ...
            db.query(sql, [orgId, userId], (err2) => {
              if (err2) {
                console.error(
                  "DB error updating user organizer request:",
                  err2
                );
                return res
                  .status(500)
              ...
              console.log(`[ORGANIZER] Request created successfully for user ${userId}, organization ${orgId}`);
              notifyAdminOfRequest(userId, orgId, orgName);
              res.status(200).json({
                success: true,
                message: "Request submitted. Status set to pending.",
              });
            });
          }
        );
      ...
      if (organization_id) {
        db.query(
          ...
          (err, rows) => {
            ...
            if (!rows || rows.length === 0) {
              return res
                .status(400)
                .json({ success: false, error: "Invalid organization_id" });
            }
        ...
        if (
          !category ||
          !["sports", "academic", "social", "club"].includes(category)
        ) {
          return res.status(400).json({
            success: false,
            error: "Valid category required for new organization",
          });
        }
        ...
        db.query(
          insertOrg,
          [new_organization_name, desc, category],
          (err, result) => {
            if (err) {
...
router.patch("/request/:id", requireAuth, (req, res) => {
  ...
  db.query(
    "SELECT * FROM organizer_requests WHERE id = ? AND user_id = ?",
    [requestId, userId],
    (err, rows) => {
      if (err) {
      ...
      if (!rows || rows.length === 0) {
        return res.status(404).json({
          success: false,
          error: "Request not found or not owned by you",
        });
      }
      ...
      function applyUpdate(finalOrgId) {
        ...
        if (
          ...
          finalOrgId !== request.organization_id
        ) {
        ...
        if (setClauses.length === 0) {
          return res
            .status(400)
            .json({ success: false, error: "No changes provided" });
        }

        setClauses.push("updated_at = CURRENT_TIMESTAMP");
        values.push(requestId);

        const updateSql = `UPDATE organizer_requests SET ${setClauses.join(
          ", "
        )} WHERE id = ?`;
      ...
      if (organization_id !== undefined) {
        ...
        db.query(
          insertOrg,
          [new_organization_name, `Requested by user ${userId}`, category],
          (err, result) => {
            if (err) {
              console.error("DB error creating organization:", err);
              return res.status(500).json({
...
router.get("/events", requireApprovedOrganizer, (req, res) => {
  ...
  db.query(sql, [organizerId], async (err, results) => {
    ...
    const eventsWithTimeline = await Promise.all(
      results.map((event) => {
        return new Promise((resolve, reject) => {
          ...
          db.query(timelineSql, [event.id], (err, timelineResults) => {
            ...
            event.attendance_rate =
              event.attendance_rate && !isNaN(event.attendance_rate)
...
router.get(
  ...
  async (req, res) => {
    ...
    const userRoleSql = "SELECT role FROM users WHERE id = ?";
    db.query(userRoleSql, [organizerId], (err, userResults) => {
      if (err) {
        console.error("DB error checking user role:", err);
        return res
          .status(500)
          .json({ success: false, error: "Internal Server Error" });
      }

      const isAdmin = userResults.length > 0 && userResults[0].role === "admin";

      // 1. Verify event ownership (skip for admins)
      const ownershipSql = isAdmin
        ? "SELECT * FROM events WHERE id = ?"
        : "SELECT * FROM events WHERE id = ? AND organizer_id = ?";
      const ownershipParams = isAdmin ? [eventId] : [eventId, organizerId];

      db.query(ownershipSql, ownershipParams, (err, eventResults) => {
        if (err) {
        ...
        const metricsSql = isAdmin
          ...
          : `SELECT
            ...
            GROUP BY e.id`;
        const metricsParams = isAdmin ? [eventId] : [eventId, organizerId];

        db.query(metricsSql, metricsParams, (err, metricsResults) => {
          if (err) {
            console.error("DB error during metrics query:", err);
            return res.status(500).json({
              success: false,
              error: "Internal Server Error",
              message: "Failed to calculate event metrics.",
            });
          }
            ...
            return res.status(200).json({
              ...
              analytics: {
                ...
                metrics: {
                  ...
                  attendance_rate:
                    metrics.attendance_rate && !isNaN(metrics.attendance_rate)
                      ? Number(metrics.attendance_rate).toFixed(2)
                      : 0,
                  remaining_capacity:
                    metrics.remaining_capacity || event.tickets_available,
 ...
 * Function: Fetch a single event by ID for the authenticated organizer
 * Middleware: requireApprovedOrganizer
 */
router.get("/events/:id", requireApprovedOrganizer, (req, res) => {
  const eventId = req.params.id;
  const organizerId = req.session.userId;

  const sql = `
    SELECT id, title, description, organizer_id, organization,
           event_date, event_time, location, capacity, price,
           category, is_flagged, moderation_notes, created_at, updated_at
    FROM events
    WHERE id = ? AND organizer_id = ?
  `;

  db.query(sql, [eventId, organizerId], (err, results) => {
    if (err) {
...
router.get(
  ...
  async (req, res) => {
    ...
    // Check if user is admin (admins can view all event attendees)
    const userRoleSql = "SELECT role FROM users WHERE id = ?";
    db.query(userRoleSql, [organizerId], (err, userResults) => {
      if (err) {
        console.error("DB error during user role check:", err);
        return res.status(500).json({
          success: false,
          error: "Internal Server Error",
      ...
      db.query(ownershipSql, [eventId], (err, eventResults) => {
        ...
        if (!isAdmin && eventResults[0].organizer_id !== organizerId) {
          return res.status(403).json({
            success: false,
            error: "Unauthorized",
            message: "You do not own this event.",
    ...
    }); // Close user role check callback
  }
);
...
function notifyUserOfDecision(userId, orgId, status) {
  ...
  db.query(
    sql,
    [userId, type, title, message, userId, orgId || null, status],
    (err) => {
